...
/**
 * EN Starts the build chain for the custom dimension DimenFill from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenFill a partir de um Dp base.
 */
@Composable
fun Dp.fillDp(): DimenFill = DimenFill(this@fillDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenFill from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenFill a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.fillDp(): DimenFill = this.toFloat().dp.fillDp()
...
class DimenFill private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenFill {
        return DimenFill(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenFill {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.flsdp, .flhdp, .flwdp).
     */
     ...
     */
    @get:Composable
    val flsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val flhdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val flwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val flsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val flhdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val flwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}