...
- **`com.appdimens.dynamic.core`** — cross-cutting engine: `DimenCache` (sharded cache, init, invalidation, stable `CalcType` ordinals), `StrategyFactorRegistry` / `SharedScreenMetrics` (satellite scale registration), `MissingModule` (Maven hint map), `DimenCalculationPlumbing` (qualifier resolution, screen dp reads, aspect-ratio multiplier, internal flags not surfaced to users), `DesignScaleConstants`, `AspectRatioLookup`, percent/resize math (`PercentSpaceMath`, `ResizeMath`, `ResizeBound`, `AutoResizePercentBasis`), Compose integration (`CompositionLocals` / `AppDimensProvider`, `LocalUiModeType`, `ComposeRememberStamps`, `ComposeDimenRemember`).
