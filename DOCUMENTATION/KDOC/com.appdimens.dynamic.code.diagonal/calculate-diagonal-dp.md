...
If [ignoreMultiWindows](calculate-diagonal-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-diagonal-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicDiagonalPx](to-dynamic-diagonal-px.md) e [toDynamicDiagonalDp](to-dynamic-diagonal-dp.md).
...
Se [ignoreMultiWindows](calculate-diagonal-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-diagonal-dp.md) sem escalar.
