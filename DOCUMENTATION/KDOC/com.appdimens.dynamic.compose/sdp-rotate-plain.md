...
EN Plain rotation with **already scaled** [rotation](sdp-rotate-plain.md) and receiver: no further scaling, only the orientation branch. Use when both sides come from the same strategy (e.g. `30.sdp.sdpRotatePlain(20.sdp)`).

PT Rotação Plain com [rotation](sdp-rotate-plain.md) e recetor **já escalados**: sem nova conversão, só o ramo de orientação. Use quando ambos os lados vêm da mesma estratégia (ex.: `30.sdp.sdpRotatePlain(20.sdp)`).
