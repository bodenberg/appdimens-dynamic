...
If [ignoreMultiWindows](calculate-fill-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-fill-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicFillPx](to-dynamic-fill-px.md) e [toDynamicFillDp](to-dynamic-fill-dp.md).
...
Se [ignoreMultiWindows](calculate-fill-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-fill-dp.md) sem escalar.
