...
If [ignoreMultiWindows](calculate-auto-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-auto-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicAutoPx](to-dynamic-auto-px.md) e [toDynamicAutoDp](to-dynamic-auto-dp.md).
...
Se [ignoreMultiWindows](calculate-auto-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-auto-dp.md) sem escalar.
