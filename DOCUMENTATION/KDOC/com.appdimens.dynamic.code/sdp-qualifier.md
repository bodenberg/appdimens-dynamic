//[AppDimens SDP, HDP, WDP: Scalable Width and Height Dimensions](../../index.md)/[com.appdimens.dynamic.code](index.md)/[sdpQualifier](sdp-qualifier.md)
...
EN Extension for Int with dynamic scaling based on **Smallest Width (swDP)**. Uses the base value by default, but when the screen metric for [qualifierType](sdp-qualifier.md) is >= [qualifierValue](sdp-qualifier.md), it uses [qualifiedValue](sdp-qualifier.md) instead.

PT Extensão para Int com dimensionamento dinâmico baseado na **Smallest Width (swDP)**. Usa o valor base por padrão, mas quando a métrica de tela para [qualifierType](sdp-qualifier.md) é >= [qualifierValue](sdp-qualifier.md), usa [qualifiedValue](sdp-qualifier.md) no lugar.
