...
If [ignoreMultiWindows](calculate-scaled-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-scaled-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicScaledPx](to-dynamic-scaled-px.md) e [toDynamicScaledDp](to-dynamic-scaled-dp.md).
...
Se [ignoreMultiWindows](calculate-scaled-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-scaled-dp.md) sem escalar.
