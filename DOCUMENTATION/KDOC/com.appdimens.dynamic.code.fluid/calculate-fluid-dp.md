...
If [ignoreMultiWindows](calculate-fluid-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-fluid-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicFluidPx](to-dynamic-fluid-px.md) e [toDynamicFluidDp](to-dynamic-fluid-dp.md).
...
Se [ignoreMultiWindows](calculate-fluid-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-fluid-dp.md) sem escalar.
