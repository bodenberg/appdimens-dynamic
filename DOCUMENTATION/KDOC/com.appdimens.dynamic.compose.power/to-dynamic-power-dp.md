...
Checks [DimenCache](../com.appdimens.dynamic.core/-dimen-cache/index.md) first. On a cache hit, returns the precomputed value; otherwise, computes via [calculatePowerDpCompose](calculate-power-dp-compose.md) and stores it.
...
⚠️ **Bypass note**: when [applyAspectRatio](to-dynamic-power-dp.md) is `false` and [qualifier](to-dynamic-power-dp.md) is `SMALL_WIDTH` with `DEFAULT` inverter, the cache is bypassed internally because a raw multiply (~2 ns) is faster than the cache lookup (~5 ns). This is intentional and not a bug.
...
Consulta o [DimenCache](../com.appdimens.dynamic.core/-dimen-cache/index.md) primeiro. No acerto, retorna o Float cacheado; no miss, calcula via [calculatePowerDpCompose](calculate-power-dp-compose.md) e armazena.
