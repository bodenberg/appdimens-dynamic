...
If [ignoreMultiWindows](calculate-interpolated-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-interpolated-dp.md) unchanged so the UI does not over-scale inside a small window.
...
**Performance**: Default multiply paths may bypass snapshot-cache storage via `shouldBypassCache` (~2 ns). Heavier / non-default paths use [DimenCache](../com.appdimens.dynamic.core/-dimen-cache/index.md). See [library/PERFORMANCE.md](../../../../library/PERFORMANCE.md).
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicInterpolatedPx](to-dynamic-interpolated-px.md) e [toDynamicInterpolatedDp](to-dynamic-interpolated-dp.md).
...
Se [ignoreMultiWindows](calculate-interpolated-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-interpolated-dp.md) sem escalar.
