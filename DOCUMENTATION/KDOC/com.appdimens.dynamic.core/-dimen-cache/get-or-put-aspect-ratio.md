...
EN Exact `ln()` computed once per [DimenMetrics](../-dimen-metrics/index.md) snapshot (since 3.1.8 — the memoized lookup table was removed). Requires a positive finite input; otherwise throws `IllegalArgumentException`.

PT `ln()` exato calculado uma vez por snapshot de [DimenMetrics](../-dimen-metrics/index.md) (desde a 3.1.8). Exige entrada positiva e finita.
