...
If [ignoreMultiWindows](calculate-fit-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-fit-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicFitPx](to-dynamic-fit-px.md) e [toDynamicFitDp](to-dynamic-fit-dp.md).
...
Se [ignoreMultiWindows](calculate-fit-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-fit-dp.md) sem escalar.
