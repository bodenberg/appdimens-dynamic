...
If [ignoreMultiWindows](calculate-percent-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-percent-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicPercentPx](to-dynamic-percent-px.md) e [toDynamicPercentDp](to-dynamic-percent-dp.md).
...
Se [ignoreMultiWindows](calculate-percent-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-percent-dp.md) sem escalar.
