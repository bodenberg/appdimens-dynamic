...
1. 
   Checks [DimenCache](../com.appdimens.dynamic.core/-dimen-cache/index.md) first. On a cache hit, returns the precomputed value;     otherwise, computes via [calculateLogarithmicDpCompose](calculate-logarithmic-dp-compose.md) and stores it.
2. 
...
⚠️ **Bypass note**: when [applyAspectRatio](to-dynamic-logarithmic-dp.md) is `false` and [qualifier](to-dynamic-logarithmic-dp.md) is `SMALL_WIDTH` with `DEFAULT` inverter, the cache is bypassed internally because a raw multiply (~2 ns) is faster than the cache lookup (~5 ns). This is intentional and not a bug.
...
1. 
   Consulta o [DimenCache](../com.appdimens.dynamic.core/-dimen-cache/index.md) primeiro. No acerto, retorna o Float cacheado;     no miss, calcula via [calculateLogarithmicDpCompose](calculate-logarithmic-dp-compose.md) e armazena.
2. 
