...
If [ignoreMultiWindows](calculate-logarithmic-dp.md) is `true`, detects split-screen mode via layout flags; if active, returns [baseValue](calculate-logarithmic-dp.md) unchanged so the UI does not over-scale inside a small window.
...
PT Núcleo de escalonamento puro compartilhado por [toDynamicLogarithmicPx](to-dynamic-logarithmic-px.md) e [toDynamicLogarithmicDp](to-dynamic-logarithmic-dp.md).
...
Se [ignoreMultiWindows](calculate-logarithmic-dp.md) for `true`, detecta split-screen via flags de layout; se ativo, retorna [baseValue](calculate-logarithmic-dp.md) sem escalar.
