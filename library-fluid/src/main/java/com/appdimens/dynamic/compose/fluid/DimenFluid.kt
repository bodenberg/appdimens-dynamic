...
/**
 * EN Starts the build chain for the custom dimension DimenFluid from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenFluid a partir de um Dp base.
 */
@Composable
fun Dp.fluidDp(): DimenFluid = DimenFluid(this@fluidDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenFluid from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenFluid a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.fluidDp(): DimenFluid = this.toFloat().dp.fluidDp()
...
class DimenFluid private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenFluid {
        return DimenFluid(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenFluid {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.fsdp, .fhdp, .fwdp).
     */
     ...
     */
    @get:Composable
    val fsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val fhdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val fwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val fsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val fhdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val fwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}