...
/**
 * EN Starts the build chain for the custom dimension DimenFit from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenFit a partir de um Dp base.
 */
@Composable
fun Dp.fitDp(): DimenFit = DimenFit(this@fitDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenFit from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenFit a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.fitDp(): DimenFit = this.toFloat().dp.fitDp()
...
class DimenFit private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenFit {
        return DimenFit(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenFit {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.ftsdp, .fthdp, .ftwdp).
     */
     ...
     */
    @get:Composable
    val ftsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val fthdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val ftwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val ftsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val fthdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val ftwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}