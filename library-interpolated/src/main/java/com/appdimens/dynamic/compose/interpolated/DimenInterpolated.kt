...
/**
 * EN Starts the build chain for the custom dimension DimenInterpolated from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenInterpolated a partir de um Dp base.
 */
@Composable
fun Dp.interpolatedDp(): DimenInterpolated = DimenInterpolated(this@interpolatedDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenInterpolated from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenInterpolated a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.interpolatedDp(): DimenInterpolated = this.toFloat().dp.interpolatedDp()
...
class DimenInterpolated private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenInterpolated {
        return DimenInterpolated(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenInterpolated {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.isdp, .ihdp, .iwdp).
     */
     ...
     */
    @get:Composable
    val isdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val ihdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val iwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val isdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val ihdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val iwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}