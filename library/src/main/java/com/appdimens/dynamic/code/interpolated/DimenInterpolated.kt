 ...
 * Used by the DimenInterpolated class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenInterpolated para definir valores específicos para condições de tela.
 */
...
// EN Methods for creating the DimenInterpolated class.
// PT Métodos de criação da classe DimenInterpolated.

/**
 * EN Starts the build chain for the custom dimension DimenInterpolated from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenInterpolated a partir de um Float (Dp) base.
 */
fun Float.interpolatedScaledDp(): DimenInterpolated = DimenInterpolated(this)

/**
 * EN Starts the build chain for the custom dimension DimenInterpolated from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenInterpolated a partir de um Int (Dp) base.
 */
fun Number.interpolatedScaledDp(): DimenInterpolated = this.toFloat().interpolatedScaledDp()
 ...
 */
class DimenInterpolated private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenInterpolated {
        return DimenInterpolated(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
     ...
     */
    @JvmOverloads
    fun ignoreMultiWindows(ignore: Boolean = true): DimenInterpolated {
        return DimenInterpolated(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenInterpolated(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolated = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolated = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolated = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenInterpolated = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    /**
     * EN Resolves isdp, ihdp, and iwdp in one pass (single [UiModeType.fromConfiguration] and config read).
     * PT Resolve isdp, ihdp e iwdp numa só passagem.
     */
    fun isdpIhdpIwdpPx(context: Context): Triple<Float, Float, Float> {
        val configuration = context.resources.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        val density = context.resources.displayMetrics.density
        val isdp = resolveDpInternal(context, DpQualifier.SMALL_WIDTH, configuration, currentUiModeType) * density
        val ihdp = resolveDpInternal(context, DpQualifier.HEIGHT, configuration, currentUiModeType) * density
        val iwdp = resolveDpInternal(context, DpQualifier.WIDTH, configuration, currentUiModeType) * density
        return Triple(isdp, ihdp, iwdp)
    }
    ...
    fun isdp(context: Context): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun ihdp(context: Context): Float = px(context, DpQualifier.HEIGHT)
    fun iwdp(context: Context): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun isdpBase(context: Context): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun ihdpBase(context: Context): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun iwdpBase(context: Context): Float = resolveDp(context, DpQualifier.WIDTH)
}