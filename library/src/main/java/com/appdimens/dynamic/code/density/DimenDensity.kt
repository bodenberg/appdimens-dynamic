 ...
 * Used by the DimenDensity class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenDensity para definir valores específicos para condições de tela.
 */
...
// EN Methods for creating the DimenDensity class.
// PT Métodos de criação da classe DimenDensity.

/**
 * EN Starts the build chain for the custom dimension DimenDensity from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDensity a partir de um Float (Dp) base.
 */
fun Float.densityScaledDp(): DimenDensity = DimenDensity(this)

/**
 * EN Starts the build chain for the custom dimension DimenDensity from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDensity a partir de um Int (Dp) base.
 */
fun Number.densityScaledDp(): DimenDensity = this.toFloat().densityScaledDp()
 ...
 */
class DimenDensity private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenDensity {
        return DimenDensity(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
     ...
     */
    @JvmOverloads
    fun ignoreMultiWindows(ignore: Boolean = true): DimenDensity {
        return DimenDensity(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenDensity(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensity = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensity = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensity = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDensity = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    /**
     * EN Resolves dsdp, dhdp, and dwdp in one pass (single [UiModeType.fromConfiguration] and config read).
     * PT Resolve dsdp, dhdp e dwdp numa só passagem.
     */
    fun dsdpDhdpDwdpPx(context: Context): Triple<Float, Float, Float> {
        val configuration = context.resources.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        val density = context.resources.displayMetrics.density
        val dsdp = resolveDpInternal(context, DpQualifier.SMALL_WIDTH, configuration, currentUiModeType) * density
        val dhdp = resolveDpInternal(context, DpQualifier.HEIGHT, configuration, currentUiModeType) * density
        val dwdp = resolveDpInternal(context, DpQualifier.WIDTH, configuration, currentUiModeType) * density
        return Triple(dsdp, dhdp, dwdp)
    }
    ...
    fun dsdp(context: Context): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun dhdp(context: Context): Float = px(context, DpQualifier.HEIGHT)
    fun dwdp(context: Context): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun dsdpBase(context: Context): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun dhdpBase(context: Context): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun dwdpBase(context: Context): Float = resolveDp(context, DpQualifier.WIDTH)
}