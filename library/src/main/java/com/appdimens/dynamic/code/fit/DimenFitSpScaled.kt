...
class FitSp private constructor(
     ...
     */
    fun ftsspFthspFtwspPx(context: Context): Triple<Float, Float, Float> {
        val configuration = context.resources.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
    ...
    /**
     * EN Resolves ftsem, fthem, and ftwem in one pass (fixed Sp / no font-scale path).
     * PT Resolve ftsem, fthem e ftwem numa só passagem (Sp fixo / sem escala de fonte).
     */
    fun ftsemFthemFtwemPx(context: Context): Triple<Float, Float, Float> {
        val configuration = context.resources.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        return Triple(
    ...
    /**
     * EN Shared implementation for [resolvePx], [ftsspFthspFtwspPx], and [ftsemFthemFtwemPx].
     * PT Implementação compartilhada para [resolvePx], [ftsspFthspFtwspPx] e [ftsemFthemFtwemPx].
     */
    ...
    fun ftssp(context: Context): Float = resolvePx(context, DpQualifier.SMALL_WIDTH)
    fun fthsp(context: Context): Float = resolvePx(context, DpQualifier.HEIGHT)
    fun ftwsp(context: Context): Float = resolvePx(context, DpQualifier.WIDTH)

    /** EN Resolve final value in pixels (WITHOUT font scale). */
    fun ftsem(context: Context): Float = resolvePx(context, DpQualifier.SMALL_WIDTH, fontScaleOverride = false)
    fun fthem(context: Context): Float = resolvePx(context, DpQualifier.HEIGHT, fontScaleOverride = false)
    fun ftwem(context: Context): Float = resolvePx(context, DpQualifier.WIDTH, fontScaleOverride = false)
}