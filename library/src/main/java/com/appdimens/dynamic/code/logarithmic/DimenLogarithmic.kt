 ...
 * Used by the DimenLogarithmic class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenLogarithmic para definir valores específicos para condições de tela.
 */
...
// EN Methods for creating the DimenLogarithmic class.
// PT Métodos de criação da classe DimenLogarithmic.

/**
 * EN Starts the build chain for the custom dimension DimenLogarithmic from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenLogarithmic a partir de um Float (Dp) base.
 */
fun Float.logarithmicScaledDp(): DimenLogarithmic = DimenLogarithmic(this)

/**
 * EN Starts the build chain for the custom dimension DimenLogarithmic from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenLogarithmic a partir de um Int (Dp) base.
 */
fun Number.logarithmicScaledDp(): DimenLogarithmic = this.toFloat().logarithmicScaledDp()
 ...
 */
class DimenLogarithmic private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenLogarithmic {
        return DimenLogarithmic(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
     ...
     */
    @JvmOverloads
    fun ignoreMultiWindows(ignore: Boolean = true): DimenLogarithmic {
        return DimenLogarithmic(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenLogarithmic(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmic = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmic = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmic = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenLogarithmic = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    /**
     * EN Resolves logsdp, loghdp, and logwdp in one pass (single [UiModeType.fromConfiguration] and config read).
     * PT Resolve logsdp, loghdp e logwdp numa só passagem.
     */
    fun logsdpLoghdpLogwdpPx(context: Context): Triple<Float, Float, Float> {
        val configuration = context.resources.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        val density = context.resources.displayMetrics.density
        val logsdp = resolveDpInternal(context, DpQualifier.SMALL_WIDTH, configuration, currentUiModeType) * density
        val loghdp = resolveDpInternal(context, DpQualifier.HEIGHT, configuration, currentUiModeType) * density
        val logwdp = resolveDpInternal(context, DpQualifier.WIDTH, configuration, currentUiModeType) * density
        return Triple(logsdp, loghdp, logwdp)
    }
    ...
    fun logsdp(context: Context): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun loghdp(context: Context): Float = px(context, DpQualifier.HEIGHT)
    fun logwdp(context: Context): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun logsdpBase(context: Context): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun loghdpBase(context: Context): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun logwdpBase(context: Context): Float = resolveDp(context, DpQualifier.WIDTH)
}