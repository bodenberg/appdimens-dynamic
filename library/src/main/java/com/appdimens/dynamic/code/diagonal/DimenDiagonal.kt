 ...
 * Used by the DimenDiagonal class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenDiagonal para definir valores específicos para condições de tela.
 */
...
// EN Methods for creating the DimenDiagonal class.
// PT Métodos de criação da classe DimenDiagonal.

/**
 * EN Starts the build chain for the custom dimension DimenDiagonal from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDiagonal a partir de um Float (Dp) base.
 */
fun Float.diagonalScaledDp(): DimenDiagonal = DimenDiagonal(this)

/**
 * EN Starts the build chain for the custom dimension DimenDiagonal from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDiagonal a partir de um Int (Dp) base.
 */
fun Number.diagonalScaledDp(): DimenDiagonal = this.toFloat().diagonalScaledDp()
 ...
 */
class DimenDiagonal private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenDiagonal {
        return DimenDiagonal(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
    ...
    fun ignoreMultiWindows(ignore: Boolean = true): DimenDiagonal {
        return DimenDiagonal(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenDiagonal(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonal = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonal = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonal = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenDiagonal = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    /**
     * EN Resolves dgsdp, dghdp, and dgwdp in one pass (single [UiModeType.fromConfiguration] and config read).
     * PT Resolve dgsdp, dghdp e dgwdp numa só passagem.
     */
    fun dgsdpDghdpDgwdpPx(context: Context): Triple<Float, Float, Float> {
        val configuration = context.resources.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        val density = context.resources.displayMetrics.density
        val dgsdp = resolveDpInternal(context, DpQualifier.SMALL_WIDTH, configuration, currentUiModeType) * density
        val dghdp = resolveDpInternal(context, DpQualifier.HEIGHT, configuration, currentUiModeType) * density
        val dgwdp = resolveDpInternal(context, DpQualifier.WIDTH, configuration, currentUiModeType) * density
        return Triple(dgsdp, dghdp, dgwdp)
    }
    ...
    fun dgsdp(context: Context): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun dghdp(context: Context): Float = px(context, DpQualifier.HEIGHT)
    fun dgwdp(context: Context): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun dgsdpBase(context: Context): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun dghdpBase(context: Context): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun dgwdpBase(context: Context): Float = resolveDp(context, DpQualifier.WIDTH)
}