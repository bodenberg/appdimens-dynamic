...
/**
 * EN Starts the build chain for the custom dimension DimenPercent from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPercent a partir de um Dp base.
 */
@Composable
fun Dp.percentDp(): DimenPercent = DimenPercent(this@percentDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenPercent from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenPercent a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.percentDp(): DimenPercent = this.toFloat().dp.percentDp()
...
class DimenPercent private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenPercent {
        return DimenPercent(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenPercent {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.psdp, .phdp, .pwdp).
     */
     ...
     */
    @get:Composable
    val psdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val phdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val pwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val psdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val phdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val pwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}