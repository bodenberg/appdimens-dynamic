...
/**
 * EN Starts the build chain for the custom dimension DimenLogarithmic from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenLogarithmic a partir de um Dp base.
 */
@Composable
fun Dp.logarithmicDp(): DimenLogarithmic = DimenLogarithmic(this@logarithmicDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenLogarithmic from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenLogarithmic a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.logarithmicDp(): DimenLogarithmic = this.toFloat().dp.logarithmicDp()
...
class DimenLogarithmic private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenLogarithmic {
        return DimenLogarithmic(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenLogarithmic {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.logsdp, .loghdp, .logwdp).
     */
     ...
     */
    @get:Composable
    val logsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val loghdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val logwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val logsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val loghdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val logwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}