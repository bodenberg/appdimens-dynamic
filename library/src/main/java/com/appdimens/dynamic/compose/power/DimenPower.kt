...
/**
 * EN Starts the build chain for the custom dimension DimenPower from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPower a partir de um Dp base.
 */
@Composable
fun Dp.powerDp(): DimenPower = DimenPower(this@powerDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenPower from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenPower a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.powerDp(): DimenPower = this.toFloat().dp.powerDp()
...
class DimenPower private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenPower {
        return DimenPower(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenPower {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.pwsdp, .pwhdp, .pwwdp).
     */
     ...
     */
    @get:Composable
    val pwsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val pwhdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val pwwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val pwsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val pwhdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val pwwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}