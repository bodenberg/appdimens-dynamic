...
/**
 * EN Starts the build chain for the custom dimension DimenAuto from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenAuto a partir de um Dp base.
 */
@Composable
fun Dp.autoDp(): DimenAuto = DimenAuto(this@autoDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenAuto from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenAuto a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.autoDp(): DimenAuto = this.toFloat().dp.autoDp()
...
class DimenAuto private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenAuto {
        return DimenAuto(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenAuto {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.asdp, .ahdp, .awdp).
     */
     ...
     */
    @get:Composable
    val asdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val ahdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val awdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val asdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val ahdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val awdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}