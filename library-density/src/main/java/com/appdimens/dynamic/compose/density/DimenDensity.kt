...
/**
 * EN Starts the build chain for the custom dimension DimenDensity from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDensity a partir de um Dp base.
 */
@Composable
fun Dp.densityDp(): DimenDensity = DimenDensity(this@densityDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenDensity from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenDensity a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.densityDp(): DimenDensity = this.toFloat().dp.densityDp()
...
class DimenDensity private constructor(
    ...
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenDensity {
        return DimenDensity(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenDensity {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.dsdp, .dhdp, .dwdp).
     */
     ...
     */
    @get:Composable
    val dsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val dhdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val dsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val dhdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}