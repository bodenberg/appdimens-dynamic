 ...
 *    (AppDimens Dynamic 3.1.8 vs legacy SDPS 3.1.6 vs Chaintech SDP-SSP Compose
 *    Multiplatform 1.0.7). Each test run captures dp resolution values
 *    (1dp, 10dp, 100dp) and time per single dp call for all three libraries.
 *
 * PT Modelos de dados + fases do benchmark de 3 vias
 *    (AppDimens Dynamic 3.1.8 vs SDPS 3.1.6 legado vs Chaintech 1.0.7).
...
data class DpResolution3(
    // sdp — no AR
    val dp1AppDimens: Float, val dp1Sdps: Float, val dp1Chaintech: Float,
    val dp10AppDimens: Float, val dp10Sdps: Float, val dp10Chaintech: Float,
    val dp100AppDimens: Float, val dp100Sdps: Float, val dp100Chaintech: Float,
    // sdpa — with AR (Chaintech doesn't support sdpa)
    val dp1AppDimensAr: Float, val dp1SdpsAr: Float,
    val dp10AppDimensAr: Float, val dp10SdpsAr: Float,
    val dp100AppDimensAr: Float, val dp100SdpsAr: Float,
)
...
data class SingleDpTiming3(
    ...
    val sdpsNs: Long,
    val chaintechNs: Long,
    // sdpa — with AR (Chaintech doesn't have sdpa)
...
/**
 * EN Raw measurements captured by the composable Chaintech probe (main thread).
 * PT Medições brutas capturadas pela sonda composable da Chaintech (main thread).
 */
data class ChaintechProbeResult(
    val sdpAvgNs: Long,
    val dp1Px: Float,
    val dp10Px: Float,
    val dp100Px: Float,
