...
/**
 * EN Starts the build chain for the custom dimension DimenDiagonal from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenDiagonal a partir de um Dp base.
 */
@Composable
fun Dp.diagonalDp(): DimenDiagonal = DimenDiagonal(this@diagonalDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenDiagonal from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenDiagonal a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.diagonalDp(): DimenDiagonal = this.toFloat().dp.diagonalDp()
...
class DimenDiagonal private constructor(
    ...
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenDiagonal {
        return DimenDiagonal(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenDiagonal {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.dgsdp, .dghdp, .dgwdp).
     */
     ...
     */
    @get:Composable
    val dgsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val dghdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dgwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val dgsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val dghdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val dgwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}