...
/**
 * EN Starts the build chain for the custom dimension DimenPerimeter from a base Dp.
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPerimeter a partir de um Dp base.
 */
@Composable
fun Dp.perimeterDp(): DimenPerimeter = DimenPerimeter(this@perimeterDp)

/**
 * EN
 * Starts the build chain for the custom dimension DimenPerimeter from a base Int.
 * Usage example: `100.scaled().screen(...)`.
 *
 * PT
 * Inicia a cadeia de construção para a dimensão customizada DimenPerimeter a partir de um Int base.
 * Exemplo de uso: `100.scaled().screen(...)`.
 */
@Composable
fun Number.perimeterDp(): DimenPerimeter = this.toFloat().dp.perimeterDp()
...
class DimenPerimeter private constructor(
     ...
     */
    fun aspectRatio(enable: Boolean = true, sensitivityK: Float? = null): DimenPerimeter {
        return DimenPerimeter(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, enable, sensitivityK)
    }
    ...
    fun screen(type: DpQualifier,
               ...
               finalQualifierResolver: DpQualifier? = null,
               orientation: Orientation? = Orientation.DEFAULT,
               inverter: Inverter? = Inverter.DEFAULT): DimenPerimeter {
        val entry = CustomDpEntry(
            dpQualifierEntry = DpQualifierEntry(type, value),
            orientation = orientation,
            customValue = customValue.toFloat().dp,
     ...
     * (SMALL_WIDTH, HEIGHT, or WIDTH), determined by the access property (.prsdp, .prhdp, .prwdp).
     */
     ...
     */
    @get:Composable
    val prsdp: Dp get() = resolve(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val prhdp: Dp get() = resolve(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val prwdp: Dp get() = resolve(DpQualifier.WIDTH)
     ...
     */
    @get:Composable
    val prsdpPx: Float get() = resolvePx(DpQualifier.SMALL_WIDTH)
     ...
     */
    @get:Composable
    val prhdpPx: Float get() = resolvePx(DpQualifier.HEIGHT)
     ...
     */
    @get:Composable
    val prwdpPx: Float get() = resolvePx(DpQualifier.WIDTH)
}