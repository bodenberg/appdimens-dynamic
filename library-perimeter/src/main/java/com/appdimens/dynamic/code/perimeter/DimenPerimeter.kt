 ...
 * Used by the DimenPerimeter class to define specific values for screen conditions.
 *
 * PT
 * Representa uma entrada de dimensão customizada com qualificadores e prioridade.
 * Usada pela classe DimenPerimeter para definir valores específicos para condições de tela.
 */
...
// EN Methods for creating the DimenPerimeter class.
// PT Métodos de criação da classe DimenPerimeter.

/**
 * EN Starts the build chain for the custom dimension DimenPerimeter from a base Float (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPerimeter a partir de um Float (Dp) base.
 */
fun Float.perimeterScaledDp(): DimenPerimeter = DimenPerimeter(this)

/**
 * EN Starts the build chain for the custom dimension DimenPerimeter from a base Int (Dp).
 * PT Inicia a cadeia de construção para a dimensão customizada DimenPerimeter a partir de um Int (Dp) base.
 */
fun Number.perimeterScaledDp(): DimenPerimeter = this.toFloat().perimeterScaledDp()
 ...
 */
class DimenPerimeter private constructor(
     ...
     */
    @JvmOverloads
    fun applyAspectRatio(apply: Boolean = true): DimenPerimeter {
        return DimenPerimeter(initialBaseDp, sortedCustomEntries, ignoreMultiWindows, apply, customSensitivityK)
    }
     ...
     */
    @JvmOverloads
    fun ignoreMultiWindows(ignore: Boolean = true): DimenPerimeter {
        return DimenPerimeter(initialBaseDp, sortedCustomEntries, ignore, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        return DimenPerimeter(initialBaseDp, reorderEntries(entry), ignoreMultiWindows, applyAspectRatio, customSensitivityK)
    }
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeter = screen(uiModeType, qualifierType, qualifierValue, orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeter = screen(type, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        orientation: Orientation? = Orientation.DEFAULT,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeter = screen(type, value, customValue.toFloat(), finalQualifierResolver, orientation, inverter)
    ...
    fun screen(
        ...
        finalQualifierResolver: DpQualifier? = null,
        inverter: Inverter? = Inverter.DEFAULT
    ): DimenPerimeter = screen(orientation, customValue.toFloat(), finalQualifierResolver, inverter)
    ...
    /**
     * EN Resolves prsdp, prhdp, and prwdp in one pass (single [UiModeType.fromConfiguration] and config read).
     * PT Resolve prsdp, prhdp e prwdp numa só passagem.
     */
    fun prsdpPrhdpPrwdpPx(context: Context): Triple<Float, Float, Float> {
        val configuration = context.resources.configuration
        val currentUiModeType = DimenCache.getCachedUiModeType(context)
        val density = context.resources.displayMetrics.density
        val prsdp = resolveDpInternal(context, DpQualifier.SMALL_WIDTH, configuration, currentUiModeType) * density
        val prhdp = resolveDpInternal(context, DpQualifier.HEIGHT, configuration, currentUiModeType) * density
        val prwdp = resolveDpInternal(context, DpQualifier.WIDTH, configuration, currentUiModeType) * density
        return Triple(prsdp, prhdp, prwdp)
    }
    ...
    fun prsdp(context: Context): Float = px(context, DpQualifier.SMALL_WIDTH)
    fun prhdp(context: Context): Float = px(context, DpQualifier.HEIGHT)
    fun prwdp(context: Context): Float = px(context, DpQualifier.WIDTH)

    /** EN Get the resolved value in DP (as Float). */
    fun prsdpBase(context: Context): Float = resolveDp(context, DpQualifier.SMALL_WIDTH)
    fun prhdpBase(context: Context): Float = resolveDp(context, DpQualifier.HEIGHT)
    fun prwdpBase(context: Context): Float = resolveDp(context, DpQualifier.WIDTH)
}